// AnalyticsPage.tsx - v2.0 - Sem banner e sem sombras
...
export function AnalyticsPage() {
  ...
  const temporalChartData = temporalTrends.map(t => ({
    date: new Date(t.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }),
    precisão: parseFloat(t.precision.toFixed(1)),
    observações: t.observations,
  }));

  // Dados do gráfico de distribuição
  const distributionData = [
    { name: 'Perfeito (<5%)', value: overallStats.perfectPredictions, color: '#10b981' },
    { name: 'Bom (<10%)', value: overallStats.goodPredictions - overallStats.perfectPredictions, color: '#3b82f6' },
    { name: 'OK (<20%)', value: overallStats.okPredictions - overallStats.goodPredictions, color: '#f59e0b' },
    { name: 'Ruim (≥20%)', value: overallStats.poorPredictions, color: '#ef4444' },
  ];
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  {((overallStats.goodPredictions / overallStats.totalObservations) * 100).toFixed(0)}% do total
                </p>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-4 gap-2 p-2 bg-gray-100 rounded-lg">
            <button
              ...
              style={{
                backgroundColor: activeTab === 'overview' ? '#001f3d' : '#e5e7eb',
                color: activeTab === 'overview' ? '#ffc72c' : '#6b7280',
                border: activeTab === 'overview' ? '2px solid #ffc72c' : '1px solid #d1d5db',
            ...
            <button
              ...
              style={{
                backgroundColor: activeTab === 'spots' ? '#001f3d' : '#e5e7eb',
                color: activeTab === 'spots' ? '#ffc72c' : '#6b7280',
                border: activeTab === 'spots' ? '2px solid #ffc72c' : '1px solid #d1d5db',
                fontWeight: activeTab === 'spots' ? '600' : '500',
                padding: '10px 16px',
                borderRadius: '6px',
                cursor: 'pointer',
                transition: 'all 0.2s'
            ...
            <button
              ...
              style={{
                backgroundColor: activeTab === 'timeline' ? '#001f3d' : '#e5e7eb',
                color: activeTab === 'timeline' ? '#ffc72c' : '#6b7280',
                border: activeTab === 'timeline' ? '2px solid #ffc72c' : '1px solid #d1d5db',
            ...
            <button
              ...
              style={{
                backgroundColor: activeTab === 'pnboia' ? '#001f3d' : '#e5e7eb',
                color: activeTab === 'pnboia' ? '#ffc72c' : '#6b7280',
                border: activeTab === 'pnboia' ? '2px solid #ffc72c' : '1px solid #d1d5db',
                fontWeight: activeTab === 'pnboia' ? '600' : '500',
                padding: '10px 16px',
                borderRadius: '6px',
                cursor: 'pointer',
                transition: 'all 0.2s'
          ...
          {activeTab === 'overview' && <div className="space-y-6">
            ...
            <Card className="p-6">
              ...
              {temporalChartData.length > 0 ? (
                ...
                <div className="h-64 flex items-center justify-center text-gray-400">
                  Sem dados suficientes para gráfico temporal
                </div>
              )}
              ...
              {temporalTrends.length >= 2 && (
                <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                  <p className="text-sm text-blue-900">
                    <strong>📊 Tendência:</strong> Sistema evoluiu de{' '}
                    <strong>{temporalTrends[0].precision.toFixed(0)}%</strong> para{' '}
                    <strong>{temporalTrends[temporalTrends.length - 1].precision.toFixed(0)}%</strong> de precisão
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card className="p-6">
                <h3 className="text-lg font-semibold mb-4">🎯 Distribuição de Previsões</h3>
              ...
              <Card className="p-6">
                  ...
                  {topSpots.map((spot, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Badge variant={spot.precision >= 95 ? 'default' : 'secondary'} className="bg-green-100 text-green-800">
                ...
                {topSpots.length === 0 && (
                  <p className="text-sm text-gray-400 text-center py-8">
                    Ainda não há picos com observações suficientes
                  </p>
                )}
              </Card>
            </div>
          ...
          {/* TAB 2: Por Pico */}
          {activeTab === 'spots' && <div className="space-y-6">
            <Card className="p-6">
              <h3 className="text-lg font-semibold mb-4">🎯 Prioridades de Calibração</h3>
          ...
          {activeTab === 'timeline' && <div className="space-y-6">
            <Card className="p-6">
              <h3 className="text-lg font-semibold mb-4">📅 Observações ao Longo do Tempo</h3>
              {temporalChartData.length > 0 ? (
                <>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={temporalChartData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="observações" fill="#3b82f6" />
                    </BarChart>
                  </ResponsiveContainer>
          ...
          {/* TAB 4: PNBOIA */}
          {activeTab === 'pnboia' && <div className="space-y-6">
            {pnboiaImpact ? (
              <>
                <Card className="p-6">
                  <h3 className="text-lg font-semibold mb-4">⚓ Impacto do PNBOIA</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-3xl font-bold text-green-900">
                          +{pnboiaImpact.improvement.toFixed(0)} pontos percentuais
                        </p>
                      </div>
                      <div>
...
// Componente para card de pico na lista de calibração
...
// Componente de análise detalhada de um pico específico
  ...
  return (
    <AdminLayout currentPage="analytics">
      <div className="space-y-6">
        ...
        {/* Ajuste Atual e Recomendação */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Ajuste Atual */}
          {spotStats.currentAdjustment && (
            <Card className="p-6">
              <h3 className="text-lg font-semibold mb-4">⚙️ Ajuste Atual</h3>
              <div className="space-y-3">
                <div>
                  <p className="text-sm text-gray-600">Shoaling Factor</p>
                  <p className="text-2xl font-bold text-gray-900">
                    {spotStats.currentAdjustment.shoalingFactor.toFixed(2)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-2">Multiplicadores por Direção</p>
                  <div className="space-y-1">
                    {spotStats.currentAdjustment.directionMultipliers.map((dm, idx) => (
                      <p key={idx} className="text-sm text-gray-700">
                        {dm.direction}: <strong>{dm.multiplier.toFixed(2)}</strong>
                      </p>
                    ))}
                  </div>
                </div>
                <div className="pt-3 border-t">
                  <p className="text-sm text-gray-600">Multiplicador Total (exemplo)</p>
                  <p className="text-xl font-bold text-gray-900">
                    {spotStats.currentAdjustment.totalMultiplier.toFixed(2)}
                  </p>
                </div>
              </div>
            </Card>
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-blue-200">
                  <p className="text-sm text-blue-800 mb-2">
                    📋 Aplicar manualmente em <code className="bg-blue-100 px-2 py-1 rounded text-xs">/data/spotWaveAdjustments.ts</code>
                  </p>
